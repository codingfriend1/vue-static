...
if (!Vue.prototype.$isServer) {
  if (config.googleAnalyticsId) {
    ...
    /**
     * Percent complete to qualify
     */
    // Readers
    const reader_scroll = 90
    const reader_time = 80

    // Scanners
    const scanner_scroll = 85
      ...
      beforeFirstHit () {
        riveted.init({
          ...
          eventHandler: function(active_seconds) {
            ...
            const file = window.store.file
            const words_per_minute = 275

            const predicted_reading_time = parseInt((file.wordCount / words_per_minute) * 60)

            window.active_seconds = active_seconds
            if(predicted_reading_time) {

              let reading_time_percent = Math.round((window.active_seconds / predicted_reading_time) * 100)
              ...
              if(window.active_seconds > Math.round(predicted_reading_time * 0.4)) {
